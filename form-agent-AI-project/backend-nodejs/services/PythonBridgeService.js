...
class PythonBridgeService {
    constructor() {
        ...
        this.bridgeScript = path.join(__dirname, '../python-bridge.py');
    ...
    async initialize() {
        try {
            ...
            // Test Python bridge
            const healthResult = await this.executeCommand('health');
            console.log('Python Bridge Health Check:', healthResult);
            
            // Load model info
            const modelInfo = await this.executeCommand('info');
    ...
    async getHealthStatus() {
        try {
            const result = await this.executeCommand('health');
            return {
                success: true,
                ...result,
                model_loaded: result.model_loaded || this.isModelLoaded
            };
        } catch (error) {
            return {
                success: false,
    ...
    async predictCategory(keyword) {
        try {
            ...
            const result = await this.executeCommand('predict', keyword);
            return result;
        } catch (error) {
            return {
                success: false,
