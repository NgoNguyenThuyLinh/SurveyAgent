...
class AuthService {
  ...
  async forgotPassword(email) {
    ...
    // Send email
    // Assume frontend URL is in ENV or default
    const emailSent = await emailService.sendPasswordResetEmail(email, token);

    if (!emailSent) {
      // Log critical failure but don't leak error to user? 
      // User requirements check: "If email sending fails, still respond 200... BUT log the failure loudly"
